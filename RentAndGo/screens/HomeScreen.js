...
const HomeScreen = () => {
  ...
  const [vehiculosdisponibles, setVehiculosDispoibles] = useState("Vehiculos Disponibles");
  ...
  const aplicarPress = async () => {
    try {
      const response = await fetch(
        "http://192.168.1.10:81/ProyectoCatedra_DPS_APIS/api/rent/filter.php",
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 204) {
        setApiData([]);
        setVehiculosDispoibles("No hay vehiculos");
      }else{
      const data = await response.json();
      console.log("Data:", data);
      setApiData(data);
      setVehiculosDispoibles("Vehiculos Disponibles")
      }
    } catch (error) {
  ...
  useEffect(() => {
    const apiUrl =
      "http://192.168.1.10:81/ProyectoCatedra_DPS_APIS/api/brand/all.php";
  ...
  useEffect(() => {
    const apiUrl =
      "http://192.168.1.10:81/ProyectoCatedra_DPS_APIS/api/typeOfCar/all.php";
  ...
  useEffect(() => {
    const apiUrl =
      "http://192.168.1.10:81/ProyectoCatedra_DPS_APIS/api/transmition/all.php";
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>{vehiculosdisponibles}</Text>
